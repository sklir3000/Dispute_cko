...
doc = DocxTemplate('/Users/valerijus.kracius/Documents/new.docx')
...
for index, row in data.iterrows():
    if 'ADJM' == row['Dispute Indicator']:
        ...
        doc.render(info)
        doc.save(f"/Users/valerijus.kracius/Downloads/{row['purchase_id']}_Dispute_summary_{row['action_key']}.docx")

    else:
        pass


globus_fil = data[(data['outgoing_bank_account_code'] == "ua_globus_uah") & (data['breakdown_type'] == "Chargeback (ADJM)")]
globus = globus_fil[['payout_token', 'partner_token', 'sent_date', 'Dispute Indicator']]


privat_fil = data[((data['outgoing_bank_account_code'] == "ua_privatbank_usd") |
                  (data['outgoing_bank_account_code'] == "ua_privatbank_eur") |
             (data['outgoing_bank_account_code'] == "ua_privatbank_uah")) &
                  (data['breakdown_type'] == "Chargeback (ADJM)")]
privat_bank = privat_fil[['payout_token', 'partner_token', 'sent_date', 'Dispute Indicator']]


birlesik_fil = data[(data['outgoing_bank_account_code'] == "tr_bo_lie_try") & (data['breakdown_type'] == "Chargeback (ADJM)")]
birlesik = birlesik_fil[['payout_token', 'partner_token', 'sent_date', 'Dispute Indicator']]

bluecash_fil = data[(data['outgoing_bank_account_code'] == "pl_bluecash") & (data['breakdown_type'] == "Chargeback (ADJM)")]
bluecash = bluecash_fil[['payout_token', 'partner_token', 'sent_date', 'Dispute Indicator']]

with open('/Users/valerijus.kracius/Downloads/partner_email.txt','w') as file:
    file.write(f"Birlesik:\n")

    file.write(f"Hello team, please provide proofs of payments for: \n")
    for index1, row1 in birlesik.iterrows():
        if 'ADJM' == row1['Dispute Indicator']:
    ...
    file.write(f"Hello team, please provide proofs of payments for: \n")
    for index1, row1 in bluecash.iterrows():
        if 'ADJM' == row1['Dispute Indicator']:
            file.write(f"{row1['payout_token']}, {row1['partner_token']}, {row1['sent_date']}\n")
    file.write(f'Thank you in advance!\n')
    ...
    file.write(f"Hello team, please provide proofs of payments for: \n")
    for index1, row1 in globus.iterrows():
        if 'ADJM' == row1['Dispute Indicator']:
            file.write(f"{row1['payout_token']}, {row1['partner_token']}, {row1['sent_date']}\n")
    file.write(f'Thank you in advance!\n')
